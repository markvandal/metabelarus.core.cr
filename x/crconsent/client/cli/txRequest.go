...
func CmdCreateRequest() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgCreateRequest(clientCtx.GetFromAddress().String(), string(argsInitiator), string(argsRecipient), string(argsRequestType), string(argsStatus), int32(argsValue), string(argsMemo), string(argsPromoUrl), string(argsCreationDt), string(argsFinalDt))
...
func CmdUpdateRequest() *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			msg := types.NewMsgUpdateRequest(clientCtx.GetFromAddress().String(), id, string(argsInitiator), string(argsRecipient), string(argsRequestType), string(argsStatus), int32(argsValue), string(argsMemo), string(argsPromoUrl), string(argsCreationDt), string(argsFinalDt))
			if err := msg.ValidateBasic(); err != nil {
				return err
			}
			return tx.GenerateOrBroadcastTxCLI(clientCtx, cmd.Flags(), msg)
