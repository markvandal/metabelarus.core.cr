...
// NewHandler creates an sdk.Handler for all the mbpasstrust type messages
func NewHandler(k keeper.Keeper) sdk.Handler {
	return func(ctx sdk.Context, msg sdk.Msg) (*sdk.Result, error) {
		ctx = ctx.WithEventManager(sdk.NewEventManager())
		switch msg := msg.(type) {
	// this line is used by starport scaffolding # 1
		case types.MsgCreateAllowance:
			return handleMsgCreateAllowance(ctx, k, msg)
		case types.MsgSetAllowance:
			return handleMsgSetAllowance(ctx, k, msg)
		case types.MsgDeleteAllowance:
			return handleMsgDeleteAllowance(ctx, k, msg)
